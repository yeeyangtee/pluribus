...
class CardInfoLutBuilder(CardCombos):
    ...
    def process_river_ehs(self, public: np.ndarray) -> np.ndarray:
        ...
        game = GameUtility(our_hand=our_hand, board=board, cards=self._cards, evaluator=self._evaluator)
        return self.simulate_get_ehs(game)

    def process_river_ehs_abstract(self, public: list) -> np.ndarray:
        """
        Get the expected hand strength for a particular card combo.
...
class CardInfoLutBuilderAbstract(CardCombosAbstract):
    """
    Stores info buckets for each street when called
    ...
    def _compute_river_clusters(self, river, n_river_clusters: int):
        """Compute river clusters and create lookup table."""
        log.info("Starting computation of river clusters.")
        start = time.time()
            ...
            self._river_ehs = list(
                tqdm(
                    executor.map(
                        self.process_river_ehs,
                        river,
                        chunksize=len(river) // 64,
                        # chunksize=64000, #16384
                    ),
                    total=len(river),
                )
            )
    ...
    def process_river_ehs(self, public: list) -> np.ndarray:
        """
        Get the expected hand strength for a particular card combo.
...
#     # Create cardlut object (barebones)
#     cardlut = CardInfoLutBuilderAbstract(n_simulations_river=n_simulations_river,
#             n_simulations_turn=n_simulations_turn,
#             n_simulations_flop=n_simulations_flop,
#             low_card_rank=low_card_rank,
#             high_card_rank=high_card_rank,
#             save_dir='/hdd1/yeeyang/pluribus/tmp',)
#     cardlut.card_info_lut['river'] = cardlut._compute_river_clusters(50)
